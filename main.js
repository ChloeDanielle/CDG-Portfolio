...
const hasHash = typeof initialHash === "string" && initialHash.length > 1;
...
if (document.body.classList.contains("is-loading") && !hasHash) {
  window.scrollTo(0, 0);
}

window.addEventListener("load", () => {
  const delay = prefersReducedMotion ? 180 : 850;
  const revealDelay = hasHash ? 0 : delay;
...
if (!prefersReducedMotion && !isCoarsePointer) {
  ...
  function animateTrail() {
    ...
    trailDots.forEach((dot, i) => {
      ...
      dot.el.style.opacity = String(1 - i / TRAIL_COUNT);
      const scale = 1 - i * 0.06;
      dot.el.style.width = `${8 * scale}px`;
      dot.el.style.height = `${8 * scale}px`;
      prevX = dot.x;
      prevY = dot.y;
    });
